...
int main(int argc, char* argv[]){
	...
	while (true) {
		...
		// counting the number of 4-prong star subgraphs (middle vertex connected to 4 others)
		// # in Kn = n * (n-1)C4
		...
		mpz_divexact_ui(val, val, 24);
		mpq_set_z(valQ, val);
		mpq_set(values[dimension-1], valQ);
		
		printf("num %d-stars in K_%d: ", 4, size);
