...
int main(int argc, char* argv[]){
	...
	Matrix<mpq_t> system = Matrix<mpq_t>(dimension + 1);
	MPList<mpq_t> values = MPList<mpq_t>(dimension + 1);
	MPList<mpq_t> terms = MPList<mpq_t>(dimension + 1); 
	mpz_t edges, temp, val;
	mpz_inits(edges, temp, val, NULL);
	Matrix<mpz_t> cycle = Matrix<mpz_t>::FromCompleteGraph(size);
	...
	for (int basisIndex = 0; basisIndex < dimension; basisIndex++) {
		...
		for (int i = 0; i < dimension; i++){
			mpq_set(system.Index(dimension, i), terms[i]);
		}

		Matrix<mpq_t> combined_q = Matrix<mpq_t>(combined.Rows, combined.Columns);
		combined.ToRationalMatrix(combined_q);
		Matrix<mpq_t>::Determinant(combined_q, values[dimension]);
		mpq_out_str(stdout, 10, values[dimension]);
	...
	printf("-----iteration %d-----\n", dimension+1);
	Matrix<mpz_t> k8 = Matrix<mpz_t>::FromCompleteGraph(9);
	basis8(k8,terms);
    mpq_set_ui(terms[dimension], 126, 1); // how many times k4 appears in k8
	terms.Print();
	...
	Matrix<mpq_t>::Determinant(k8_q, values[dimension]);
